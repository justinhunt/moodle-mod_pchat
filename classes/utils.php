...
class utils{
  ...
  public static function get_expiredays_options(){
    ...
    //prepare URLs for use in displaying topic media
    public static function fetch_topicmedia_urls($topic, $modulecontext){
        $fs = get_file_storage();
        $files = $fs->get_area_files($modulecontext->id,  constants::M_COMPONENT,constants::M_TOPICMEDIA,$topic->id);
        $urls=[];
        foreach ($files as $file) {
            $filename = $file->get_filename();
            if($filename=='.'){continue;}
            $filepath = '/';
            $mediaurl = \moodle_url::make_pluginfile_url($modulecontext->id, constants::M_COMPONENT,
                constants::M_TOPICMEDIA, $topic->id,
                $filepath, $filename);
